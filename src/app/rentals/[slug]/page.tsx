...
import { rentalBySlugQuery } from '@/sanity/lib/queries'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const rental: RentalVehicle = await client.fetch(rentalBySlugQuery, { slug })

  if (!rental) return { title: 'Rental Vehicle Not Found' }

  return {
    title: `${rental.year} ${rental.brand} ${rental.model} Rental | Modern Auto Garage`,
    description: `Rent this ${rental.year} ${rental.brand} ${rental.model} in Manchester, NH. ${rental.mileage} miles, ${rental.fuelType}, ${rental.transmission}. Daily Rate: $${rental.pricePerDay.toLocaleString('en-US')}/day.`,
  }
}
...
export default async function RentalDetailPage({ params }: PageProps) {
  ...
  }).format(rental.pricePerDay)

  const formattedMileage = new Intl.NumberFormat('en-US').format(rental.mileage)

  return (
    <main className="min-h-screen bg-gray-50/50 pb-24">
      <RentalStructuredData rental={rental} />
      ...
      <div className="border-b border-gray-200 bg-white py-4 shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-400">
            ...
            <Link href="/rentals" className="hover:text-primary transition-colors">Rentals</Link>
            <ChevronRight className="w-3 h-3 text-gray-400" />
            <span className="text-black font-extrabold">{rental.brand} {rental.model}</span>
          </nav>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-gray-200 pb-8">
          <div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-black tracking-tight mb-4">
              {rental.year} {rental.brand} <span className="text-primary italic">{rental.model}</span>
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-[2.5rem] border border-gray-200 shadow-sm">
              <ImageGallery images={rental.images} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-black text-white p-6 rounded-[2rem] shadow-md border border-gray-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wide text-white">Clean & Sanitized</h4>
                  <p className="text-[11px] text-gray-400">Deep clean before every rental</p>
                </div>
              </div>
              <div className="flex items-center gap-3 border-t sm:border-t-0 sm:border-l border-white/10 pt-4 sm:pt-0 sm:pl-6">
                <FileText className="w-6 h-6 text-primary shrink-0" />
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wide text-white">Flexible Rental</h4>
                  <p className="text-[11px] text-gray-400">Daily, weekly, or monthly</p>
                </div>
              </div>
              <div className="flex items-center gap-3 border-t sm:border-t-0 sm:border-l border-white/10 pt-4 sm:pt-0 sm:pl-6">
                <Sparkles className="w-6 h-6 text-primary shrink-0" />
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wide text-white">Full Tank Ready</h4>
                  <p className="text-[11px] text-gray-400">Serviced & fueled up</p>
                </div>
              </div>
            </div>
            ...
            <section className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-gray-200 shadow-sm relative overflow-hidden">
              ...
              <div className="prose prose-lg max-w-none text-gray-600 leading-relaxed font-medium">
                {rental.description ? (
                  <PortableText value={rental.description} />
                ) : (
                  <p>No description available for this rental vehicle. Contact Modern Auto Garage for booking info and requirements.</p>
                )}
              </div>
            </section>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="sticky top-28 space-y-6">
              ...
              <div className="bg-gradient-to-b from-gray-900 via-black to-black text-white rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden border border-gray-800">
                ...
                <p className="text-gray-400 font-extrabold uppercase tracking-widest text-[10px] mb-2">Rental Rate</p>
                <h3 className="text-5xl lg:text-6xl font-black mb-2 tracking-tight text-white">{formattedPrice}</h3>
                <p className="text-gray-400 text-xs font-medium mb-6">per day + local tax & fees</p>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-center justify-between text-sm py-2">
                    ...
                    <span className="font-extrabold text-white">{rental.year}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm py-2">
                    <span className="text-gray-400 flex items-center gap-2"><Settings2 className="w-4 h-4" /> Transmission</span>
                    <span className="font-extrabold text-white">{rental.transmission}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm py-2">
                  ...
                  <div className="flex items-center justify-between text-sm py-2">
                    ...
                    <span className="font-extrabold text-white">{rental.fuelType}</span>
                  </div>
                </div>
                ...
                {rental.status === 'Available' && (
                  <div className="space-y-3">
                    ...
                    <a 
                      href={`https://wa.me/16036224428?text=${encodeURIComponent(`Hello Modern Auto Garage, I am interested in renting the ${rental.year} ${rental.brand} ${rental.model} (Rate: ${formattedPrice}/day). Is it available for booking?`)}`}
                      target="_blank"
              ...
              <div className="bg-white rounded-[2.5rem] p-8 border border-gray-200 shadow-sm">
                <h4 className="text-xl font-black mb-6 text-black">Rental Support</h4>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <a href="tel:+16036224428" className="font-extrabold text-black text-base hover:text-primary transition-colors">(603) 622-4428</a>
                    </div>
                  </div>
